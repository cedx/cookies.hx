...
class Cookies extends EventTarget {
  ...
  function get_keys(): Array<String> {
    final keys = Browser.document.cookie.replace(~/((?:^|\s*;)[^=]+)(?=;|$)|^\s*|\s*(?:=[^;]*)?(?:\1|$)/g, '');
    return keys.length ? keys.split(~/\s*(?:=[^;]*)?;\s*/).map(StringTools.urlDecode) : [];
  }
  ...
  **/
  public function get(key: String, ?defaultValue: String): Null<String> {
    ...
    try {
      ...
      return Browser.document.cookie.replace(scanner, '$1').urlDecode();
    }
  ...
  **/
  ...
  public function has(key: String): Bool {
    final token = key.urlEncode().replace(~/[-.+*]/g, '\\$&');
    return new EReg('(?:^|;\\s*)$token\\s*=').test(Browser.document.cookie);
  }
  ...
  **/
  public function set(key: String, value: String, ?options: CookieOptions): Cookies {
    ...
    final oldValue = get(key);
    Browser.document.cookie = cookieValue;
  ...
  **/
  ...
  private function emit(key: Null<String>, oldValue: Null<String>, newValue: Null<String>): Void
    dispatchEvent(new StorageEvent('change', {
      key: key,
      newValue: newValue,
      oldValue: oldValue,
      storageArea: Browser.document.cookie,
      url: Browser.location.href
    }));

  /** Merges the default cookie options with the specified ones. **/
  private function getOptions(?options: CookieOptions): CookieOptions {
  ...
  private function removeItem(key: String, ?options: CookieOptions): Void {
    if (!has(key)) return;
    final cookieOptions = getOptions(options);
    cookieOptions.expires = Date.fromTime(0);
    Browser.document.cookie = '${key.urlEncode()}=; $cookieOptions';
  }
...
private class CookieIterator {
  ...
  private final keys: ArrayIterator<String>;
