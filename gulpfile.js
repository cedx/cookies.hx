...
export async function lint() {
	await exec("lix", ["run", "checkstyle", "--config", "etc/checkstyle.json", "--source src", "--source test"]);
...
export async function test() {
	log("Testing with `haxe.Json` serializer...");
	await exec("haxe", ["test.hxml"]);
	log("Testing with `tink.Json` serializer...");
	await exec("haxe", ["--library", "tink_json", "test.hxml"]);
}
